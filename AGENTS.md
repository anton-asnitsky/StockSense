...
- On Windows, run framework commands through `./scripts/aidlc.ps1` if `aidlc`
  is not on PATH. Launch Codex with `./scripts/Start-Codex.ps1` for hook access.
- Project setup customizations intentionally inherit user models and permissions.
  Reapply these adaptations after reviewing any upstream runtime refresh.
- Installation is not authorization to provision cloud resources or send orders.

- The upstream prerequisite descriptions above describe shipped defaults.
  StockSense intentionally overrides them: inherit the user's model/provider,
  retain the host sandbox/network policy, and omit broad Git permission grants.
