...
| SS-34 | Integrate and evaluate authorized RAG | SS-22, SS-33 | Retrieval adapter enforces tenant/document authorization; both approved embedding candidates are compared on held-out multilingual queries for same-language/cross-language retrieval quality, citations, CPU latency and memory; prompt injection, cross-tenant, stale/deleted-source cases pass; embedding model and cost limits are explicit; vector restore/rebuild is demonstrated |
...
| Embedding default and named demo languages | SS-33/34 | Multilingual documents/queries are required; select initial languages and compare EmbeddingGemma-300M with Qwen3-Embedding-0.6B using same-language and cross-language tests (ADR 0015) |
